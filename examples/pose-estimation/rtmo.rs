...
pub fn config(args: &RtmoArgs) -> Result<Config> {
    let config = match args.scale {
        Scale::T => Config::rtmo_t(),
        Scale::S => Config::rtmo_s(),
        Scale::M => Config::rtmo_m(),
        Scale::L => Config::rtmo_l(),
        _ => anyhow::bail!("Unsupported RTMO scale: {}", args.scale),
    }
