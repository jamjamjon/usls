...
pub fn config(args: &DwposeArgs) -> Result<Config> {
    let config = match args.scale {
        Scale::T => Config::dwpose_133_t(),
        Scale::S => Config::dwpose_133_s(),
        Scale::M => Config::dwpose_133_m(),
        Scale::L => Config::dwpose_133_l(),
        _ => anyhow::bail!("Unsupported DWPose scale: {}", args.scale),
    }
