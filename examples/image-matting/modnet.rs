...
#[derive(Args, Debug)]
pub struct ModnetArgs {
    /// Variant: photographic
    #[arg(long, default_value = "photographic")]
    pub variant: String,
...
pub fn config(args: &ModnetArgs) -> Result<Config> {
    let config = match args.variant.as_str() {
        "photographic" => Config::modnet_photographic(),
        _ => anyhow::bail!("Unsupported MODNet variant: {}", args.variant),
    }
