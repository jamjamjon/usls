...
#[derive(Args, Debug)]
pub struct DoclayoutYoloArgs {
    /// Variant: docstructbench
    #[arg(long, default_value = "docstructbench")]
    pub variant: String,
...
pub fn config(args: &DoclayoutYoloArgs) -> Result<Config> {
    let mut config = match args.variant.as_str() {
        "docstructbench" => Config::doclayout_yolo_docstructbench(),
        _ => anyhow::bail!("Unsupported DocLayout-YOLO variant: {}", args.variant),
    }
