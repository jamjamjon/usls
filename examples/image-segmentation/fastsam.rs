...
pub fn config(args: &FastsamArgs) -> Result<Config> {
    let mut config = match args.scale.parse()? {
        Scale::S => Config::fastsam_s(),
        Scale::X => Config::fastsam_x(),
        _ => anyhow::bail!("Unsupported FastSAM scale: {}. Try s, x.", args.scale),
    }
