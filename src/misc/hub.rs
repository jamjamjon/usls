...
use crate::{retry, Dir, PREFIX_LENGTH};
...
/// ```rust,ignore
/// let path = usls::Hub::default().try_fetch("images/bus.jpg")?;
...
/// ```rust,ignore
/// let hub = usls::Hub::default().with_owner("jamjamjon").with_repo("usls");
...
impl Hub {
    ...
    pub fn try_fetch(&mut self, s: &str) -> Result<String> {
        ...
        } else if let Some((owner_, repo_, tag_, file_name_)) = Self::is_valid_github_release_url(s)
        {
            ...
            let saveout = self
                .to
                .path_with_subs(&[&owner_, &repo_, &tag_])?
                .join(&file_name_);
    ...
    pub(crate) fn is_valid_github_release_url(
        url: &str,
