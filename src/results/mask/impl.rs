...
use crate::{contour, BorderType, InstanceMeta, MaskStyle, Polygon};

/// Mask: Gray Image.
#[derive(Builder, Default, Clone)]
pub struct Mask {
    /// `ImageBuffer<Luma<u8>, Vec<u8>>`
    mask: GrayImage, // TODO: ImageBuffer<Luma<f32>, Vec<f32>>
    meta: InstanceMeta,
...
impl Mask {
    ...
    pub fn polygons(&self) -> Vec<Polygon> {
        let contours = contour::find_contours_with_threshold(&self.mask, 0);
        let polygons: Vec<Polygon> = contours
            .into_par_iter()
            .filter_map(|c| {
                if c.border_type == BorderType::Hole && c.points.len() <= 2 {
                    return None;
                }
                let coords: Vec<[f32; 2]> =
                    c.points.iter().map(|p| [p.0 as f32, p.1 as f32]).collect();
