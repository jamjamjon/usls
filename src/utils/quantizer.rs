...
impl Quantizer {
    ...
    fn quantize_internal(&self, input: &[f32], size: (usize, usize)) -> Vec<usize> {
        ...
        match input.len() {
            4 => vec![
                self.quantize_value(input[0], size_per_bin_w, bins_w),
                self.quantize_value(input[1], size_per_bin_h, bins_h),
                self.quantize_value(input[2], size_per_bin_w, bins_w),
                self.quantize_value(input[3], size_per_bin_h, bins_h),
            ],
            2 => vec![
                self.quantize_value(input[0], size_per_bin_w, bins_w),
                self.quantize_value(input[1], size_per_bin_h, bins_h),
    ...
    pub fn quantize(&self, input: &[f32], size: (usize, usize)) -> Vec<usize> {
