        ...
        let ys: Vec<Y> = xs[0]
            ...
            .filter_map(|logits| {
                let logits = if self.apply_softmax {
                    ...
                    let logits_data = logits.to_vec::<f32>().ok()?;
                    let max_val = logits_data.iter().fold(f32::NEG_INFINITY, |a, &b| a.max(b));
                    let exps: Vec<f32> = logits_data.iter().map(|&x| (x - max_val).exp()).collect();
                    let sum_exps: f32 = exps.iter().sum();
                    exps.into_iter().map(|x| x / sum_exps).collect::<Vec<f32>>()
                } else {
                    logits.to_vec::<f32>().ok()?
                };
