...
use crate::{
    contour, Config, Contour, DynConf, Engine, Engines, FromConfig, Hbb, Image, ImageProcessor,
    Model, Module, NmsOps, Ops, Polygon, Xs, X, Y,
};
...
impl Model for YOLOPv2 {
    ...
    fn run(&mut self, engines: &mut Engines, images: Self::Input<'_>) -> Result<Vec<Y>> {
        let x = crate::perf!("YOLOPv2::preprocess", self.processor.process(images)?);
        let ys = crate::perf!("YOLOPv2::inference", engines.run(&Module::Model, &x)?);
        crate::perf!("YOLOPv2::postprocess", self.postprocess(&ys))
    }
}
