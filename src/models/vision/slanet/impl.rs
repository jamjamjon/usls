...
use crate::{
    elapsed_module, models::BaseModelVisual, ort_inputs, Config, Image, Keypoint, Text, X, Y,
};
...
impl SLANet {
    ...
    pub fn forward(&mut self, xs: &[Image]) -> Result<Vec<Y>> {
        ...
        let ys = elapsed_module!("SLANet", "inference", self.inference(ys)?);
    ...
    fn inference(&mut self, xs: X) -> Result<(X, X)> {
        let output = self.base.engine_mut().run(ort_inputs![xs]?)?;
        Ok((
    ...
    fn postprocess(&self, xs: (X, X)) -> Result<Vec<Y>> {
        ...
        let (bboxes_all, structures_all) = xs;
        for (bid, (bboxes, structures)) in bboxes_all
            .axis_iter(Axis(0))
            ...
            .enumerate()
        {
            ...
            for (i, structure) in structures.axis_iter(Axis(0)).enumerate() {
                ...
                if self.td_tokens.contains(&token) {
                    let slice_bboxes = bboxes.slice(s![i, ..]).to_owned();
