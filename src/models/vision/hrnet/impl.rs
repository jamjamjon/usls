...
/// HRNet: top-down heatmap-based human pose estimation
///
/// Supports COCO 17-keypoint body and COCO-WholeBody 133-keypoint estimation with the
/// `w32` / `w48` backbones. The model takes a person crop and outputs per-keypoint
/// heatmaps of shape `[batch, num_keypoints, h, w]` (1/4 of the input resolution).
...
impl Model for HRNet {
    ...
    fn run(&mut self, engines: &mut Engines, input: Self::Input<'_>) -> Result<Vec<Y>> {
        let images = input;
        let (xs, centers_and_scales) = crate::perf!("HRNet::preprocess", self.preprocess(images)?);
        let ys = crate::perf!("HRNet::inference", engines.run(&Module::Model, &xs)?);
        let y = crate::perf!("HRNet::postprocess", {
...
impl HRNet {
    ...
    fn postprocess(
        ...
        let heatmaps = X::from(x0);

        // [batch, num_keypoints, hm_h, hm_w]
        ...
        let results: Vec<Y> = (0..total_crops)
            ...
            .map(|batch_idx| {
                ...
                let x_factor = scale.x() / hm_w as f32;
                let y_factor = scale.y() / hm_h as f32;
                let x_offset = center.x() - scale.x() * 0.5;
                let y_offset = center.y() - scale.y() * 0.5;

                let keypoints: Vec<Keypoint> = (0..self.nk)
                    .map(|kpt_idx| {
                        let hm = heatmaps.slice(s![batch_idx, kpt_idx, .., ..]);
                        let (py, px, confidence) = Self::heatmap_argmax(&hm);

                        if confidence > self.kconfs[kpt_idx] {
                            // Sub-pixel refinement: shift 0.25 toward the larger neighbor.
                            let (mut fx, mut fy) = (px as f32, py as f32);
                            if px > 0 && px < hm_w - 1 && py > 0 && py < hm_h - 1 {
                                let dx = hm[[py, px + 1]] - hm[[py, px - 1]];
                                let dy = hm[[py + 1, px]] - hm[[py - 1, px]];
                                fx += dx.signum() * 0.25;
                                fy += dy.signum() * 0.25;
                            }

                            let x = fx * x_factor + x_offset;
                            let y = fy * y_factor + y_offset;
    ...
    fn heatmap_argmax(hm: &ndarray::ArrayView2<f32>) -> (usize, usize, f32) {
        let mut max_val = f32::MIN;
        let (mut max_y, mut max_x) = (0usize, 0usize);
        for ((y, x), &val) in hm.indexed_iter() {
            if val > max_val {
                max_val = val;
                max_y = y;
                max_x = x;
