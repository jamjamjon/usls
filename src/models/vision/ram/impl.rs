...
impl Model for Ram {
    ...
    fn run(&mut self, engines: &mut Engines, images: Self::Input<'_>) -> Result<Vec<Y>> {
        let x = crate::perf!("RAM::preprocess", self.processor.process(images)?);
        let ys = crate::perf!("RAM::inference", engines.run(&Module::Model, &x)?);
        crate::perf!("RAM::postprocess", self.postprocess(&ys))
    }
}
