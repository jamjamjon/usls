        ...
        let task: Option<Task> = match config.task() {
            ...
            None => match engine.try_fetch("task") {
                Some(x) => match x.as_str() {
                    ...
                    x => {
                        anyhow::bail!("Unsupported YOLO Task: {x}");
                    }
                },
                None => None,
            },
                ...
                (Some(task), Some(version)) => {
                    let layout = match (&task, version) {
                        ...
                        (Task::InstanceSegmentation, Version(26, 0, _)) => {
                            YOLOPredsFormat::n_a_xyxy_confcls_coefs().apply_nms(false)
                        }
                        (
...
impl YOLO {
    pub(crate) fn postprocess(&self, outputs: &Xs) -> Result<Vec<Y>> {
        ...
        let f = |(idx, pred): (usize, _)| -> Option<Y> {
            ...
            let f_bbox = |(i, bbox): (usize, ndarray::ArrayViewD<'_, f32>)|
             -> Option<(Option<Hbb>, Option<Obb>)> {
                // confidence & class_id
                ...
                // filter by class id
                if !self.classes_retained.is_empty() && !self.classes_retained.contains(&class_id) {
                    return None;
                }
            ...
            let (y_hbbs, y_obbs) = if use_rayon_for_candidates {
                slice_bboxes
                    .axis_iter(Axis(0))
                    .into_par_iter()
                    .enumerate()
        ...
        let use_rayon = if let Task::InstanceSegmentation = self.task {
            preds.len_of(Axis(0)) != 1
        } else {
            preds.len_of(Axis(0)) >= 10
        };

        let ys: Vec<Y> = if use_rayon {
            preds
                .axis_iter(Axis(0))
                .into_par_iter()
                .enumerate()
