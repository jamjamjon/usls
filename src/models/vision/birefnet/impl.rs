...
impl Model for BiRefNet {
    ...
    fn run(&mut self, engines: &mut Engines, images: Self::Input<'_>) -> Result<Vec<Y>> {
        let x = crate::perf!("BiRefNet::preprocess", self.processor.process(images)?);
        let ys = crate::perf!("BiRefNet::inference", engines.run(&Module::Model, &x)?);
        crate::perf!("BiRefNet::postprocess", self.postprocess(&ys))
    }
}
    ...
    fn postprocess(&self, outputs: &Xs) -> Result<Vec<Y>> {
        ...
        let ys: Vec<Y> = xs
            ...
            .filter_map(|(idx, luma)| {
                ...
                let luma = luma.mapv(|x| 1. / ((-x).exp() + 1.));
