...
impl Florence2 {
    pub fn new(mut config: Config) -> Result<Self> {
        let vision_encoder = Engine::from_config(config.take_module(&crate::Module::Visual)?)?;
        let text_embed = Engine::from_config(config.take_module(&crate::Module::Textual)?)?;
        let encoder = Engine::from_config(config.take_module(&crate::Module::TextualEncoder)?)?;

        // Handle decoder and decoder_merged
        let (decoder, decoder_merged) =
            match config.take_module(&crate::Module::TextualDecoderMerged) {
                Ok(merged_cfg) => {
                    let decoder =
                        Engine::from_config(config.take_module(&crate::Module::TextualDecoder)?)?;
                    let decoder_merged = Engine::from_config(merged_cfg)?;
                    (decoder, decoder_merged)
                }
                Err(_) => {
                    // Use same config for both
                    let decoder_cfg = config.take_module(&crate::Module::TextualDecoder)?;
                    let decoder = Engine::from_config(decoder_cfg.clone())?;
                    let decoder_merged = Engine::from_config(decoder_cfg)?;
                    (decoder, decoder_merged)
                }
            };
    ...
    fn encode_text(&mut self, task: &Task, images: &[Image]) -> Result<X> {
        let xs = images
            ...
            .map(|im| {
                ...
                let ids = self.text_processor.encode_text_ids(&text, true)?;
    ...
    fn generate_then_decode(
        ...
        let mut decoder_outputs = {
            ...
            // Convert all outputs to owned X
            (0..ys.len())
        ...
        for _ in 0..self.max_length {
            ...
            // let (finished, last_tokens) = self.decoder_merged.image_processor().par_generate(
            //     logits,
            //     &mut token_ids,
            //     self.eos_token_id,
            // )?;
