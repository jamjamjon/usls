...
use crate::{
    elapsed_module, Config, DynConf, Engine, Hbb, Image, Mask, Ops, Processor, Sam3Prompt, Task,
    Xs, X, Y,
};
...
impl SAM3 {
    pub fn new(config: Config) -> Result<Self> {
        let task = config.task().cloned().unwrap_or(Task::Sam3Image);
        ...
        let (textual_encoder, geometry_encoder, text_batch, geo_batch, spec) = match task {
            Task::Sam3Image => {
                let te = Engine::try_from_config(&config.textual_encoder)?;
                let ge = Engine::try_from_config(&config.encoder)?;
                let tb = te.batch().opt();
                let gb = ge.batch().opt();
                (Some(te), Some(ge), tb, gb, "sam3-image".to_string())
            }
            Task::Sam3Tracker => (None, None, 1, 1, "sam3-tracker".to_string()),
    ...
    fn encode_texts(&mut self, texts: &[&str]) -> Result<Vec<(X, X)>> {
        ...
        for chunk in texts.chunks(self.text_batch) {
            ...
            let ys = textual_encoder.run(Xs::from(vec![input_ids, attention_mask]))?;
    ...
    fn encode_geometry_batch(
            ...
            .map(|(i, c)| (i * self.geo_batch, c))
        {
            ...
            let ys = geometry_encoder.run(Xs::from(vec![
                input_boxes,
                input_labels,
                fpn_feat_batch,
                fpn_pos_batch,
    ...
    fn decode_tracker_single_object(
        ...
        let iou_scores = &decoder_outputs[0];
        let pred_masks = &decoder_outputs[1];
        let obj_scores = &decoder_outputs[2];
