...
/// ```ignore
/// // Owned inputs
/// let inputs = ort_inputs![x1, x2]?;
///
/// // Borrowed inputs
/// let inputs = ort_inputs![&x1, &x2]?;
///
/// // Zero-copy views
/// let inputs = ort_inputs![x1.view(), x2.view()]?;
///
/// // Mixed inputs (owned + borrowed + view)
/// let inputs = ort_inputs![x1, &x2, x3.view()]?;
///
/// // By name
/// let inputs = ort_inputs!["images" => x1, "masks" => &x2]?;
/// ```
#[macro_export]
macro_rules! ort_inputs {
    // Positional inputs
    ($($t:expr),+ $(,)?) => {{
        (|| -> ::anyhow::Result<_> {
            Ok([ $(
